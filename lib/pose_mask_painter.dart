...
class PoseMaskPainter extends CustomPainter {
  ...
  final maskPaint = Paint()
    ..colorFilter = const ColorFilter.mode(Color.fromRGBO(0, 0, 255, 0.5), BlendMode.srcOut);
  ...
  void _paintMask(Canvas canvas, Size size) {
    ...
    canvas.drawImageRect(mask!, Rect.fromLTWH(0, 0, mask!.width.toDouble(), mask!.height.toDouble()),
        Rect.fromLTWH(0, 0, size.width, size.height), maskPaint);
  }

  @override
  bool shouldRepaint(PoseMaskPainter oldDelegate) {
    return oldDelegate.pose != pose || oldDelegate.mask != mask || oldDelegate.imageSize != imageSize;
  }
  ...
  static PoseLandmark getPoselandMark(PoseLandmarkType type, Pose? detectedPose) {
    late PoseLandmark landmark;
    for (final it in detectedPose!.landmarks) {
      if (it.type == type) landmark = it;
    }
    return landmark;
  }
}
